...
# CREATE TABLES
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays(songplay_id SERIAL PRIMARY KEY, start_time numeric NOT NULL, user_id int NOT NULL, level varchar , song_id varchar , artist_id varchar, session_id int NOT NULL, location varchar, user_agent varchar);
""")
# create dimension tables
# user in app
# can't duplicate add the user with same first name and last name
...
""")
# song in music db
# can't duplicate add same title song
...
""")
#timestamps of records in songplays broken down into specific unit
time_table_create = ("""CREATE TABLE IF NOT EXISTS time(start_time numeric UNIQUE PRIMARY KEY, hour int, day int, week int, month int, year int, weekday varchar);
""")
...
user_table_insert = ("""INSERT INTO users(user_id,first_name,last_name,gender,level) VALUES (%s, %s, %s, %s,%s) ON CONFLICT (user_id) DO UPDATE SET level = users.level
""")
...
song_select = (""" SELECT song_id,artists.artist_id FROM songs,    
                  artists WHERE (songs.artist_id = artists.artist_id AND songs.title = %s 
